...
import { getPieceFileName } from "./util";
import { messageTypes } from "./constants";
import type { TurnType } from "chesster.js/dist/types";
...
const aiWorker = new Worker("worker.bundle.js");

const fen: string = "";

if (fen !== "") game.init(fenToGameState(fen));
...
var selectedPieceElement: HTMLElement | null = null;
...
function updateLastMove(gameHistory: number[]) {
    for (let i = 0; i < BOARD_SIZE; i++) {
...
function updateVisualizeMove(move: number) {
    clearVisualizations();
...
function initBoard(gameBoard: number[]) {
    for (let i = 0; i < BOARD_SIZE; i++) {
        if (gameBoard[i] === 0) continue;

        elementBoard[i].piece.classList.add(getPieceFileName(gameBoard[i]));
        ...
        if (
            (game.white.isCheckmated && gameBoard[i] === pieces.WHITE_KING) ||
            (game.black.isCheckmated && gameBoard[i] === pieces.BLACK_KING)
        ) {
            ...
            (game.white.isChecked && gameBoard[i] === pieces.WHITE_KING) ||
            (game.black.isChecked && gameBoard[i] === pieces.BLACK_KING)
        ) {
            let img = document.createElement("img");
            img.setAttribute(
                "src",
    ...
    undo.disabled =
        ...
        (game.isGameOver()
            ? false
            : enableAI
              ? game.turn !== playerTeam
              : false);
}

function updateBoard(gameBoard: number[], previousBoard: number[]) {
    for (let i = 0; i < BOARD_SIZE; i++) {
        if (gameBoard[i] !== previousBoard[i]) {
            if (previousBoard[i] !== 0)
                elementBoard[i].piece.classList.remove(
                    getPieceFileName(previousBoard[i])
                );

            if (gameBoard[i] !== 0)
                elementBoard[i].piece.classList.add(
                    getPieceFileName(gameBoard[i])
                );
        }
...
function updateIndicators() {
    for (let i = 0; i < BOARD_SIZE; i++) {
        ...
        if (
            (game.white.isCheckmated && game.board[i] === pieces.WHITE_KING) ||
            (game.black.isCheckmated && game.board[i] === pieces.BLACK_KING)
        ) {
            let img = document.createElement("img");
            img.setAttribute(
                "src",
            ...
            (game.white.isChecked && game.board[i] === pieces.WHITE_KING) ||
            (game.black.isChecked && game.board[i] === pieces.BLACK_KING)
        ) {
            let img = document.createElement("img");
            img.setAttribute(
                "src",
...
async function clientMove(move: number) {
    await makeMove(move);
    if (enableAI && !game.isGameOver())
        aiWorker.postMessage({
            type: messageTypes.MOVE,
            state: game.getState(),
        }); // disable to enable two player
}
...
async function makeMove(move: number) {
    ...
    updateLastMove(game.turnHistory);
        ...
        return new Promise<void>((resolve) => {
            ...
            elementBoard[from].piece.style.transform = piecesAreRotated
                ? `translate(${
                      ...
                      elementBoard[to].cell.offsetTop -
                      elementBoard[from].cell.offsetTop
                  }px) rotate(180deg)`
                : `translate(${
                      elementBoard[to].cell.offsetLeft -
                      elementBoard[from].cell.offsetLeft
...
function makeUndo() {
    ...
    if (enableAI && game.turn !== playerTeam && game.turnHistory.length === 0)
        aiWorker.postMessage({
            type: messageTypes.MOVE,
            state: game.getState(),
        });
    ...
    switch (event.data.type) {
        case messageTypes.MOVE:
            if (event.data.move === undefined) {
                console.error(
                    "ai returned undefined move, likely game is over"
                );
                return;
            }
    ...
    (() => {
        function touchHandler(event: any) {
            ...
            if (selectedMoves.length > 0) {
                ...
                if (selectedMoves.length > 1) {
                    if (((selectedMoves[0] >>> 7) & 0b1) !== 1)
                        throw new Error(
                            "invalid multi-move type: " +
                                selectedMoves[0].toString(2)
                        );
                    ...
                    for (const move of selectedMoves) {
                        ...
                        const promotionHandler = (event: any) => {
                            event.preventDefault();
            ...
            for (let move of moves) {
                ...
                elementBoard[(move >>> 8) & 0b111111].move.classList.add(
                    "selected"
                );
            }
...
function restartGame() {
    ...
    if (importedFEN !== "") game.init(fenToGameState(importedFEN));
    else game.init();
...
var loadCustomGameTimeout: ReturnType<typeof setTimeout>;
...
const maxChessBOARD_SIZE = 980;
const borderlessThreshold = 600;

var isBorderless = false;

function getChessBOARD_SIZE() {
    ...
    if (size > maxChessBOARD_SIZE) size = maxChessBOARD_SIZE;
    else if (
